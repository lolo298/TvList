import type { Image_Type } from 'database';
...
export function getImagePath<T extends Image_Type>(
	path: string,
	_format?: T,
	size?: ImageSizeParam<T>
): string {
	console.log('Getting image path:', path, 'Format:', _format, 'Size:', size);
	if (!path) {
		return '';
	}
