...
import { PrismaClient, type Show, Image_Type } from '@prisma/client';
...
export async function getMovie(id: number) {
	...
	const res = await prisma.show.create({
		data: {
			...
			images: {
				create: [{ url: data.poster_path, type: Image_Type.POSTER }]
			}
		}
	});
	return res as Show;
...
export async function getSerie(id: number) {
	...
	const seasonsCreate = await Promise.all(
		data.seasons.map(async (season) => {
			...
			return {
				...
				images: {
					create: seasonData.poster_path ? [{ url: seasonData.poster_path, type: Image_Type.POSTER }] : []
				},
				episodes: {
					create: seasonData.episodes.map((episode) => ({
						...
						name: episode.name,
						images: {
							create: episode.still_path ? [{ url: episode.still_path, type: Image_Type.STILL }] : []
						},
	...
	const res = await prisma.show.create({
		data: {
			...
			images: {
				create: [
					{ url: data.poster_path, type: Image_Type.POSTER },
					{ url: data.backdrop_path, type: Image_Type.BACKDROP }
				]
			},
			seasons: {
